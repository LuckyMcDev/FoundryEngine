...
Builders are the primary way to create and register game content in Foundry Engine.
Each builder wraps a NeoForge registry and provides a fluent API for configuring your
items, blocks, recipes, sounds, and particles.

All builders implement `BuilderBase<T>`, which extends `Supplier<T>` and provides:

| Method                 | Description                                               |
|------------------------|-----------------------------------------------------------|
| `build()`              | Build the object without registering it                   |
| `get()`                | Get the registered object (throws if not registered)      |
...
`de.luckymcdev.foundryengine.api.builder.item.ItemBuilder`

Creates custom `Item` instances. The default factory produces `EngineItem`, which
supports callback-based event hooks.
...
Item callbacks let you add behavior without creating a Java subclass.
...
```groovy
// Basic item with lore and rarity
...
`de.luckymcdev.foundryengine.api.builder.block.BlockBuilder`

Creates custom `Block` instances (default: `EngineBlock`). Optionally creates a
corresponding `BlockItem`.
...
```groovy
// Simple block with custom properties
...
`de.luckymcdev.foundryengine.api.builder.recipe.RecipeBuilder`

Creates all recipe types. Uses a `RecipeResult` record that pairs a recipe ID with
a save function.
...
`de.luckymcdev.foundryengine.api.builder.sound.SoundBuilder`
...
`de.luckymcdev.foundryengine.api.builder.particle.ParticleBuilder`
...
By default, each builder auto-generates asset/data files (models, recipes, blockstates,
sound JSONs, etc.). You can disable this per-builder:
...
To disable globally, pass `false` to `shouldGenerateData()`.

## See Also

- [Registries](registries.md) — How builders interact with the registry system
- [Events](events.md) — Adding behavior via event hooks
- [Blueprints](blueprints.md) — Visual scripting with builtin event nodes
