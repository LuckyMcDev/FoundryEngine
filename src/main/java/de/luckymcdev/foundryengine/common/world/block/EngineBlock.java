...
public class EngineBlock extends Block {
    ...
    public void animateTick(BlockState state, Level level, BlockPos pos, RandomSource random) {
        ...
        if (cb != null) cb.run(state, level, pos, random);
    ...
    public void destroy(LevelAccessor level, BlockPos pos, BlockState state) {
        ...
        if (cb != null) cb.run(level, pos, state);
        else super.destroy(level, pos, state);
    }

    @Override
    public void wasExploded(ServerLevel level, BlockPos pos, Explosion explosion) {
        WasExplodedCallback cb = get(CallbackType.WAS_EXPLODED);
        if (cb != null) cb.run(level, pos, explosion);
        else super.wasExploded(level, pos, explosion);
    }

    @Override
    public void stepOn(Level level, BlockPos pos, BlockState onState, Entity entity) {
        StepOnCallback cb = get(CallbackType.STEP_ON);
        if (cb != null) cb.run(level, pos, onState, entity);
        else super.stepOn(level, pos, onState, entity);
    }

    @Override
    public void setPlacedBy(Level level, BlockPos pos, BlockState state, @Nullable LivingEntity by, ItemStack itemStack) {
        SetPlacedByCallback cb = get(CallbackType.SET_PLACED_BY);
        if (cb != null) cb.run(level, pos, state, by, itemStack);
        else super.setPlacedBy(level, pos, state, by, itemStack);
    }

    @Override
    public void fallOn(Level level, BlockState state, BlockPos pos, Entity entity, double fallDistance) {
        FallOnCallback cb = get(CallbackType.FALL_ON);
        if (cb != null) cb.run(level, state, pos, entity, fallDistance);
    ...
    public void playerDestroy(Level level, Player player, BlockPos pos, BlockState state, @Nullable BlockEntity blockEntity, ItemStack tool) {
        ...
        if (cb != null) cb.run(level, player, pos, state, blockEntity, tool);
    ...
    public void handlePrecipitation(BlockState state, Level level, BlockPos pos, Biome.Precipitation precipitation) {
        ...
        if (cb != null) cb.run(state, level, pos, precipitation);
        else super.handlePrecipitation(state, level, pos, precipitation);
    }
