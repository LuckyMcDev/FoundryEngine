...
public class EngineItemDisplay extends Display.ItemDisplay implements Attackable, Targeting {
    ...
    private DisplayHitboxUtil.HitboxBounds hitboxBounds =
            new DisplayHitboxUtil.HitboxBounds(-0.5f, -0.5f, -0.05f, 0.5f, 0.5f, 0.05f);
    private boolean pickable = true;

    public EngineItemDisplay(EntityType<?> type, Level level) {
        super(type, level);
    }
    ...
    protected AABB makeBoundingBox(Vec3 position) {
        if (hitboxBounds == null) {
            return new AABB(position.x - 0.5, position.y - 0.5, position.z - 0.5,
    ...
    private void refreshHitboxFromTransformation() {
        this.hitboxBounds = DisplayHitboxUtil.forItem(getCurrentTransformation());
    ...
    public void setDisplayItemTransform(ItemDisplayContext context) {
        setItemTransform(context);
    }
    ...
    public interface InteractionConsumer {
        InteractionResult onInteract(EngineItemDisplay display, Player player, InteractionHand hand, Vec3 location);
    }

    @FunctionalInterface
    public interface AttackConsumer {
        void onAttack(EngineItemDisplay display, Player player);
    }
}