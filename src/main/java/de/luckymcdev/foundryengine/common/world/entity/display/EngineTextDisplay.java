...
public class EngineTextDisplay extends Display.TextDisplay implements EngineDisplay {
    ...
    private void refreshHitbox() {
        ...
        hitboxBounds = DisplayHitboxUtil.forText(new Transformation(
    ...
    public void setShadow(boolean shadow) {
        byte cur = currentFlags();
        setFlags(shadow ? (byte) (cur | FLAG_SHADOW) : (byte) (cur & ~FLAG_SHADOW));
    }

    public void setSeeThrough(boolean seeThrough) {
        byte cur = currentFlags();
        setFlags(seeThrough ? (byte) (cur | FLAG_SEE_THROUGH) : (byte) (cur & ~FLAG_SEE_THROUGH));
    }

    public void setUseDefaultBackground(boolean useDefault) {
        byte cur = currentFlags();
        setFlags(useDefault ? (byte) (cur | FLAG_USE_DEFAULT_BACKGROUND) : (byte) (cur & ~FLAG_USE_DEFAULT_BACKGROUND));
    }

    public void setAlignment(Align alignment) {
        byte cur = (byte) (currentFlags() & ~FLAG_ALIGN_LEFT & ~FLAG_ALIGN_RIGHT);
        setFlags(switch (alignment) {
            case LEFT -> (byte) (cur | FLAG_ALIGN_LEFT);
            case RIGHT -> (byte) (cur | FLAG_ALIGN_RIGHT);
            case CENTER -> cur;
        });
    }
