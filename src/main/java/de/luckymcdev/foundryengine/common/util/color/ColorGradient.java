...
public class ColorGradient {
    ...
    public Color getColor(float progress) {
        if (steps.isEmpty()) return Color.WHITE;
        if (steps.size() == 1 || progress <= steps.get(0).pos) return steps.get(0).color;
        if (progress >= steps.get(steps.size() - 1).pos) return steps.get(steps.size() - 1).color;
