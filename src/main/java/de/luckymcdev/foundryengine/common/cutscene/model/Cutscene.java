...
public class Cutscene {
	...
	public ArrayList<CommandAttachment> getCommandAttachments() {
		...
		for (CutsceneAttachment att : this.attachments) {
			if (att instanceof CommandAttachment cmd) {
				commands.add(cmd);
			}
		}
		return commands;
	...
	private void ensureAnchorRotations() {
		...
		if (anchorRotations.size() == 2 && anchors > 2) {
			...
			if (anchorRotations.size() < anchors) {
				...
				while (anchorRotations.size() < anchors) {
					anchorRotations.add(last);
				}
		...
		while (anchorHoldTicks.size() < anchors) {
			anchorHoldTicks.add(0);
		}
		while (anchorHoldTicks.size() > anchors) {
			anchorHoldTicks.removeLast();
		}
	}
	...
	public int getAnchorHoldTicks(int anchorIndex) {
		ensureAnchorRotations();
		if (anchorIndex < 0 || anchorIndex >= anchorHoldTicks.size()) {
			return 0;
		}
