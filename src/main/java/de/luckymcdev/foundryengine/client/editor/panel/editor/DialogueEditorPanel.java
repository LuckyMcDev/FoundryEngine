...
public class DialogueEditorPanel extends EditorPanel {
	...
	private void renderStyleEditor(DialogueTree tree, ImGraphicsExtractor g) {
		...
		if (ImGui.isItemHovered()) {
			ImGui.setTooltip("Restore this tree's default colors and sizes");
		}
		...
		if (ImGui.treeNodeEx("Dialogue Box", ImGuiTreeNodeFlags.DefaultOpen)) {
			...
			if (c != null) {
				s.setDialogueBackground(c);
			}
			c = g.colorEdit4("Border##dialogueBorder", s.getDialogueBorder());
			if (c != null) {
				s.setDialogueBorder(c);
			}

			var bw = new ImInt(s.getDialogueBorderWidth());
			ImGui.setNextItemWidth(100);
			if (ImGui.inputInt("Border Width##dbw", bw)) {
				s.setDialogueBorderWidth(Math.max(0, bw.get()));
			}
			ImGui.treePop();
		}
		...
		if (ImGui.treeNodeEx("Options Box", ImGuiTreeNodeFlags.DefaultOpen)) {
			...
			if (c != null) {
				s.setOptionsBackground(c);
			}
			c = g.colorEdit4("Border##optionsBorder", s.getOptionsBorder());
			if (c != null) {
				s.setOptionsBorder(c);
			}

			var bw = new ImInt(s.getOptionsBorderWidth());
			ImGui.setNextItemWidth(100);
			if (ImGui.inputInt("Border Width##obw", bw)) {
				s.setOptionsBorderWidth(Math.max(0, bw.get()));
			}
			ImGui.treePop();
		}
		...
		if (ImGui.treeNodeEx("Overlay & Text", ImGuiTreeNodeFlags.None)) {
			...
			ImGui.setNextItemWidth(100);
			if (ImGui.inputInt("Speaker Font", iv)) {
				s.setSpeakerFontSize(Math.max(1, iv.get()));
			}
			iv.set(s.getDialogueFontSize());
			ImGui.setNextItemWidth(100);
			if (ImGui.inputInt("Dialogue Font", iv)) {
				s.setDialogueFontSize(Math.max(1, iv.get()));
			}
			iv.set(s.getOptionFontSize());
			ImGui.setNextItemWidth(100);
			if (ImGui.inputInt("Option Font", iv)) {
				s.setOptionFontSize(Math.max(1, iv.get()));
			}
			ImGui.treePop();
		}
		...
		if (ImGui.treeNodeEx("Layout", ImGuiTreeNodeFlags.None)) {
			...
			ImGui.setNextItemWidth(100);
			if (ImGui.inputInt("Margin", iv)) {
				s.setMargin(Math.max(0, iv.get()));
			}
			iv.set(s.getPanelHeight());
			ImGui.setNextItemWidth(100);
			if (ImGui.inputInt("Panel Height", iv)) {
				s.setPanelHeight(Math.max(20, iv.get()));
			}
			iv.set(s.getButtonHeight());
			ImGui.setNextItemWidth(100);
			if (ImGui.inputInt("Button Height", iv)) {
				s.setButtonHeight(Math.max(10, iv.get()));
			}
			iv.set(s.getOptionGap());
			ImGui.setNextItemWidth(100);
			if (ImGui.inputInt("Option Gap", iv)) {
				s.setOptionGap(Math.max(0, iv.get()));
			}
			ImGui.treePop();
		}
	...
	private void renderNewNodeForm(DialogueTree tree) {
		...
		ImGui.sameLine();
		if (ImGui.button(ImIcons.TIMES + "##cancelNode")) {
			showNewNode = false;
		}
	...
	private void renderNodeEdit(DialogueTree tree, ImGraphicsExtractor g) {
		...
		if (ImGui.isItemHovered()) {
			ImGui.setTooltip("Speaker name color");
		}
	...
	private void renderNewOptionForm(DialogueNode node) {
		...
		ImGui.sameLine();
		if (ImGui.button(ImIcons.TIMES + "##cancelOption")) {
			showNewOption = false;
		}
