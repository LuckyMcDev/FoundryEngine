...
public final class EditorAutocomplete {
	...
	private static final Color COL_SELECTED = Color.ofABGR(0xFF2A5A8A);
	private static final Color COL_HOVERED = Color.ofABGR(0xFF244C74);
	private static final Color COL_TEXT = Color.ofABGR(0xFFD4D4D4);
	private static final Color COL_TYPE = Color.ofABGR(0xFF707070);
	private static final Color COL_SCROLLBG = Color.ofABGR(0x40FFFFFF);
	private static final Color COL_SCROLLFG = Color.ofABGR(0x80FFFFFF);
	...
	static String currentWord(EditorCoordinates cursor, List<List<EditorGlyph>> lines) {
		...
		for (int i = start; i < end && i < line.size(); i++) {
	...
	public boolean render(float charWidth, float lineHeight,
		...
		for (int i = 0; i < visCount; i++) {
			...
			dl.addText(tx, ty, EditorTheme.toImU32(item.color()), typeIcon(item.type()));
			tx += ICON_COL_W;
			dl.addText(tx, ty, EditorTheme.toImU32(COL_TEXT), item.text());
	...
	private void accept(EditorCoordinates cursor, List<List<EditorGlyph>> lines, ImGuiCoreTextEditor editor) {
		if (items.isEmpty() || selectedIdx >= items.size()) {
			hide();
			return;
		}
		...
		Color defColor = editor.getColorizer().getDefaultColor();
		for (char c : text.toCharArray()) {
			line.add(cursor.column++, new EditorGlyph(c, defColor));
		}

		if (addParens) {
			cursor.column--; // land inside the parens
		}
