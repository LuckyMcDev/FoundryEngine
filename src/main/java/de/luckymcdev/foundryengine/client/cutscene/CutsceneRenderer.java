...
public class CutsceneRenderer {
	...
	private static void renderCutscene(Cutscene cutscene) {
		...
		for (int i = 0; i < count; i++) {
			...
			Color color;
			if (i == 0 || i == count - 1) {
				color = new Color(base.r(), base.g(), base.b(), 0.627f);
			} else {
				color = new Color(base.r(), base.g(), base.b(), 0.439f);
			}
	...
	private static void renderBezierPoint(Vec3 eye, Vec3 look, BezierPoint point) {
		if (eye.distanceTo(point.getPos()) < 0.1) {
			return;
		}
		...
		if (point.getPath().isSinglePoint()) {
