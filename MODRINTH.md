...
Foundry Engine is a Minecraft mod that adds an in-game editor, custom rendering, and a self-contained mod packaging system. Built on NeoForge 26.1.x.x for Minecraft 26.1.x.

## Core Features

- **In-Game Editor** - A Dear ImGui-based editor overlay with 18+ dockable panels for file management, code editing, texture viewing, cutscene editing, and area management. Includes 7 UI themes.

- **Groovy Scripting Engine** - Full Groovy 5 runtime for bundle logic. Register items, blocks, recipes (all 9 types), sounds, and particles from scripts. Three-sided execution (common/client/server) with per-side entrypoints.

- **Bundle System** - A self-contained mod packaging format. Define items, blocks, recipes, sounds, models, textures, language files, and scripts in a single distributable folder. No separate mod project or Java compilation required. Bundles are discovered automatically, loaded dynamically, support dependency management, and can include their own resource and data packs.
...
- **Mesh Rendering and OBJ** - Custom 3D rendering engine with 8 render pipelines (position, color, lit models, textured, debug lines, through-walls, OBJ entity cutout, OBJ entity translucent). Built-in OBJ model loader and renderer with the Suzanne monkey head model included. Full vertex/normal/UV support and fan triangulation.
...
- **Event System** - 84 events across 16 event classes covering blocks, items, players, entities, levels, network, client, server, commands, recipes, areas, game sessions, stages, slots, and bundles. Usable from Groovy scripts, Blueprints, or Java addons.
...
- **Network System** - 18+ packet types for editor synchronization, file transfer, world state, and bundle data using codec-based serialization.
