...
class WaypointEntrypoint implements BundleEntrypoint {
    ...
    void onLoad() {
        PlayerEvents.loggedIn { event ->
            ...
            def level = player.serverLevel()
            def dim = level.dimension()
            if (!Common.getWaypointManager().isLoaded(dim)) {
                [
                    new Waypoint("Spawn", "minecraft:bed", 0, 64, 0, new Color(255, 200, 0)),
                    new Waypoint("Village", "minecraft:bell", -200, 63, 150, new Color(100, 200, 255)),
                    new Waypoint("Portal", "minecraft:ender_eye", 500, 64, -300, new Color(200, 50, 200))
